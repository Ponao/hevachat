    ...
    render() {
        return (
            <Link style={{textDecoration: 'none'}} to={`/rooms/${this.props.room._id}`}><div className="room-item" title={`Room ${this.props.room.title}`}>
                <Avatar style={{width: 40, height: 40, backgroundColor: `rgb(${this.props.room.color})`}} name={this.props.room.title} />
                <div>
                    <p>{this.props.room.isPrivate && <LockOutlinedIcon />}{this.props.room.title}</p>
                    <div className="users">
                        {this.props.room.users.map(user => {
                            return <Avatar style={{width: 16, height: 16, fontSize: 12,backgroundColor: `rgb(${user.color})`}} name={user.email} />
                        })}
                    </div>
                </div>
            </div></Link>
